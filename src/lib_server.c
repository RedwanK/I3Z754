...
int recois_numero_calcul(int client_socket_fd, char* data, char* code)
{
 ...
 while(count<tabSize) 
 {
	...
	{
	   resultat = findMini(tabNumbers, tabSize);
	   break;
	} else if(strcmp(operateur, "maximum") == 0)
	{
	   resultat = findMaxi(tabNumbers, tabSize);
	   break;
	} else if(strcmp(operateur, "ecart-type") == 0)
	{
	   resultat = findEcart(tabNumbers, tabSize);
	   break;
	} else if(strcmp(operateur, "moyenne") == 0)
	{
	   resultat = findMoy(tabNumbers, tabSize);
	   break;
	}
    }
    count += 1;
...
{
	int moy = findMoy(tab, size);
	int count = 0;
	float result= 0;
	while(count<size)
	{
	    result+=(tab[count]-moy)*(tab[count]-moy);	
	    count +=1;
	}
	result = result/size;
	return sqrt(result);
...
int recois_envoie_message(int socketfd) {
  while(1)
  {
    ...
    if(fork() == 0)
    {
      ...
      printf ("Reception : [si vous recevez plusieurs messages d'affilé, répondez en LIFO] %s\n", data);
