...
int valideJson(char *message, int from)
{
    ...
    if(dataConsistency(code, message, from) != 0)
    {
        perror("ERREUR DE COHERENCE DES DONNEES.");
        return -1;
    }
...
int checkCalcul(char *values, int from)
{
    ...
    while(token != NULL)
    {
        ...
        if(count == 0)
        {
            ...
            {
		int check = checkOperators(token);
                if(check != 0 && check != 10)
                {
...
int checkOperators(char *token)
{
    ...
    {
        return 10;
    } else if(strcmp(token, ecartype) == 0)
    {
        return 10;
    } else if(strcmp(token, moy) == 0)
    {
        return 10;
    }
